...
def gen_tcl(dut_root,ft_path, dut_folder, src_list, include, dut_name, dut_name_ext, submodule_assert, submodule_assume):
    ...
    if (not os.path.exists(tcl_filename) or override_tool_script):
        ...
        tcl.write("set AUTOCC_ROOT $env(AUTOCC_ROOT)\n\n")
        tcl.write("# Analyze design under verification files (do not edit)\n")
        ...
        tcl.write("set PROP_PATH ${AUTOCC_ROOT}/ft_"+dut_name+"/sva\n\n")
        tcl.write("set_elaborate_single_run_mode off\n")
        tcl.write("# Analyze property files\n")
        tcl.write("analyze -clear\n")
        tcl.write("analyze -sv12 -f ${AUTOCC_ROOT}/ft_"+dut_name+"/files.vc\n")
        tcl.write("# Elaborate design and properties\n")
        if tool_mode.startswith("autocc"):
            top_module = dut_name+"_wrap"
        else:
            top_module = dut_name
        tcl.write("elaborate -top " + top_module)
    ...
    if (not os.path.exists(vc_filename) or override_tool_script):
        ...
        if tool_mode.startswith("autocc"):
            vc.write("${DUT_ROOT}/"+dut_folder+ dut_name+"_wrap.sv\n");
        vc.close()
...
def gen_sby(dut_root, ft_path, dut_folder, src_list, include, dut_name, dut_name_ext, submodule_assert, submodule_assume):
    ...
    if (not os.path.exists(sby_filename)):
        ...
        for line in link_files:
            sby.write(line)
        sby.write("$AUTOCC_ROOT/ft_"+dut_name+"/sva/"+ dut_name + "_prop.sv\n")
        sby.write("$AUTOCC_ROOT/ft_"+dut_name+"/sva/"+ dut_name + "_bind.svh\n")
        if not recursive:
...
def main():
    ...
    if not dut_root:
        ...
        sys.exit(1)
    autocc_root = os.getenv('AUTOCC_ROOT')
    if not autocc_root:
        print("You must define AUTOCC_ROOT")
        sys.exit(1)
    ...
    if tool_mode.startswith("autocc"):
        rtl = open(dut_path, "r+")
        wrap = open(dut_root+"/"+ dut_name + "_wrap.sv", "w+")
        create_wrapper(rtl, wrap)
        rtl.close()
    
    if tool_mode != "autocc_only_wrap":
        ...
        if (num_lines == 0) and (tool_mode=="autocc"):
            prop.write("wire architectural_state_eq = 1'b1;\n")
        ...
        print("\nSUCCESS!\nYou can now run the following command to start the formal verification:")
        print("jg ft_"+dut_name+"/FPV.tcl\n")
main()